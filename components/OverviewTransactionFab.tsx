...
export function OverviewTransactionFab({
  ...
  return <>
    <button type="button" onClick={openPopup} aria-label="Add transaction" className="focus-ring fixed bottom-20 right-5 z-40 flex h-14 items-center gap-2 rounded-full bg-indigo-600 px-5 font-bold text-white shadow-xl transition hover:-translate-y-0.5 hover:bg-indigo-700 lg:bottom-7 lg:right-7">
    ...
    {open && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="overview-add-title" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
      <form onSubmit={submit} className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="space-y-4 p-5">
          ...
          <div className="border-t border-slate-200 pt-4"><p className="mb-3 text-xs font-black uppercase tracking-wider text-slate-500">Daily cash reconciliation</p><div className="grid gap-3 sm:grid-cols-3">
        ...
        <footer className="flex justify-end gap-2 border-t bg-slate-50 px-5 py-4"><button type="button" onClick={() => setOpen(false)} className="rounded-lg border border-slate-300 px-4 py-2.5 text-xs font-bold">Cancel</button><button type="submit" disabled={saving} className="rounded-lg bg-emerald-600 px-5 py-2.5 text-xs font-black text-white disabled:opacity-40">{saving ? "Saving…" : amount > 0 ? "Add & calculate" : "Save balances"}</button></footer>
      </form>
    </div>}
  </>;
}
