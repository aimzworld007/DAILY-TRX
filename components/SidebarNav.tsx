...
import { Archive, BarChart3, Clock3, FileText, LayoutDashboard, LogOut, Settings, WalletCards } from "lucide-react";
import { useAuth } from "@/components/AuthProvider";
import { getFirebaseAuth } from "@/lib/firebase";

const items = [
  { href: "/", label: "Overview", mobile: "Home", icon: LayoutDashboard },
...
export function SidebarNav() {
  ...
  return <>
    ...
    <nav className="lg:hidden fixed bottom-3 left-3 right-3 z-50 grid grid-cols-5 rounded-2xl border border-white/10 bg-[#11182b]/95 p-1.5 text-slate-400 shadow-2xl backdrop-blur-xl" aria-label="Mobile navigation">
      {items.map(({href,mobile,icon:Icon}) => <Link key={href} href={href} className={`focus-ring flex flex-col items-center gap-1 rounded-xl py-2 ${active(href) ? "bg-indigo-500/15 text-indigo-300" : "hover:text-white"}`}><Icon className="h-5 w-5"/><span className="text-[9px]">{mobile}</span></Link>)}
    </nav>
  </>;
}
